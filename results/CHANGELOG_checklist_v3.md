...
## T23-full: 占位符替换 + 新节文字（部分完成）

- 已完成：T23.2 剩余硬编码表格改为占位符（`@@TABLE_SENS@@`，生成结果与原手写七行逐字一致）；T23.6 为 T11–T20 各任务结果各加一段：§10.2（T11 op 候选与 spill 降幅、T12 ALAP、T13 采样、T14 退火及耗时超标披露，图 20/23）、§10.5（T17 命中来源拆分，图 21）、§10.7（T16 代理择优与贪心曲线，图 19、表 10-8）、§11.1（T16 重建消融，表 11-1b，含 T20 交叉验证）；`论文.md` S4 段更正大图兜底为"两次官方核实"（代理 top-1 一次 + 退火 top-1 一次），并补入采样与退火的方法描述。`fill_paper.py` 新增 `_t23_values`、`_sens_table` 及 `P2P3_*` 计数。
- 事实核对更正：初稿曾写"个别用例 P3 冠军落在对角线下方"，核对 `final.csv` 后改为 0 个（32 个相等、68 个 P3 更高）。
- 验收：`assemble_paper` → `fill_paper` 无未替换占位符 → `md2docx` 成功；`grep "几何平均"` 的 3 处均带"补充指标/比值型指标"字样。`04_实验章节.md` 中残余的 `x.xx` 形式手写数字：运行环境版本、$\beta$ 等超参数取值、`case_059/097` 单核换入换出字节数（1.69 MB、10.6 MB）、单核兜底下限 1.00、`0.0000%` 逐位验证结果，均非需由 `fill_paper` 生成的实验统计量。
- 未完成（阻塞）：T23.4 相关工作、T23.5 参考文献——依赖增强版文档，仓库内没有该文件，不编造文献。T23.3 中依赖增强版原文的插入段（§5.1′ 等）同样未做。
- 注意：§11.2 粒度、§11.3 敏感性两节的样本仍是 T03 之前的 25/18 个用例（未重跑，不在 B 档范围），正文已如实写明个数。
- 0.3 回归：P2 58984、P1 116868、搬运量模型 90/90。

## T24 / B8: B 档全量重建与最终复核

执行：`build_pool --jobs 16`（mismatches=0，51379 行，1480 s）→ `check_pool`（POOL OK）→ `make_report` → `export_plans`（1200 个方案）→ `verify_final --problems 1 2 3 --jobs 16`（**checked=1500 mismatches=0**，1855 s）→ 论文装配/填数/docx → `digest.py > results/digest_final.txt`。`solution/finalize.sh` 因 CRLF 行尾在本机 `bash` 下报 `python: command not found`，未实际执行；其各步骤（图表、装配、填数、docx、导出方案）已逐条手动执行，效果等价。

最终验收清单：
1. `check_pool.py` 打印 `POOL OK`：通过（条件 4 例外 2 个，最大相对差 0.0067%，见 `pool_condition4_exceptions.json`）。
2. `verify_final.py` `mismatches=0`：通过（1500/1500）。
3. `final.csv` 各问题各 N 的算术平均 ≥ T07 结束时（commit `6a0c890`）：通过，12 项全部不低于——P1 N=2..5 = 1.7808→1.8141、2.4957→2.5478、3.1558→3.1863、3.7144→3.7353；P2 1.8759→2.0608、2.6589→2.8809、3.3647→3.6309、3.9885→4.2405；P3 1.8832→2.0626、2.6716→2.8897、3.3969→3.6741、4.0463→4.3122。
4. `fill_paper.py` 无未替换占位符：通过（278 个占位符，残留 `@@` 为 0）。
5. `paper/论文.docx` 生成成功：通过。
6. 0.3 回归：通过（P2 58984、P1 116868、搬运量模型 90/90）。
7. `git diff --stat -- code data docs` 为空：通过。

各任务完成情况：
- 完成：T00–T08、T09（范围收窄为 P1 慢用例）、T09-proxy、T10、T11、T12、T13、T14、T16、T17（仅 T17.1/T17.2）、T20、T22（8/11 张图）、T23-core、T23-full（T23.2/T23.6 部分）、本任务。
- 跳过（B 档计划明确不做）：T09 原版、T15（基线胜平负，`fig26` 无数据）、T17.3（`l2study`，`fig22` 无数据）、T17.4（`cachemodel` 对照，`fig27` 无数据）、T18、T19.4、T21。
- 未完成（阻塞）：T23.4 相关工作、T23.5 参考文献及 T23.3 中依赖增强版原文的插入段——仓库内没有增强版文档，不编造文献；最终论文缺这几部分。
- 未做的 T22 子项：trace 甘特图（E4，`fig28`、`trace_stall.json`）。
- 已知偏差（均已在论文或 changelog 披露）：T14 退火耗时超标（最大 2015 s，42 个配置 >60 s）；T16 的 P1 消融只覆盖 96 个用例；§11.2 粒度、§11.3 敏感性沿用 T03 之前的样本（25/18 个用例，未重跑）；P1 大图（case_014/072/076/091）走代理兜底 + 两次官方核实。

关键数字（`summary.json` 算术平均加速比，N=2..5）：P1 1.8141/2.5478/3.1863/3.7353；P2 2.0608/2.8809/3.6309/4.2405；P3 2.0626/2.8897/3.6741/4.3122。

## 粒度重跑（§11.2，study30）

- 命令：`run_experiments.py --stage granularity --problems 1 2 3 --cores 4 --jobs 16 --cases <study30 去掉 case_076>`（29 个用例 × 3 问题 × 7 个 β = 609 行，26 s，多数命中评估缓存）。case_076 是 P1 大图，单次官方评估过慢而排除。旧结果备份于 `results/baseline_snapshot/granularity_before_rerun.csv`。
- 结果与旧结论**不一致**，正文据实重写：最优 β 为 问题 1/2/3 = 2 / 0.5 / 0.5（旧文称三者一致落在 0.25 的单峰）；问题 1 在扫描范围内随 β 增大单调上升，与 §6.4「粗粒度对问题 1 更有利」一致；问题 2/3 在 [0.12, 0.5] 内变化 3.2%，问题 1 达 7.7%；默认 β=0.35 在问题 1 上偏细（0.25 处损失 4.0%，0.5 处 1.3%），但多起点候选集含 β=0.6，最终成绩不受限。
- `fill_paper.py` 新增 `GRAN_N`、`GRAN_BEST_BETA`、`GRAN_LOSS_025`、`GRAN_LOSS_05`、`GRAN_PLATEAU_P23`；论文样本个数改由 `GRAN_N` 生成。
- 0.3 回归：P2 58984、P1 116868、搬运量模型 90/90；`fill_paper` 无未替换占位符，docx 生成成功。
- §11.3 敏感性仍是旧样本（18 个用例），未重跑，正文已写明个数。

## 论文通读修正

通读 `paper/论文_完整.md`（摘要、§1–§12），修正下列不一致；`fill_paper` 无未替换占位符，docx 生成成功，0.3 回归通过（58984 / 116868 / 90/90）。
1. 评估总次数：`TOTAL_EVALS` 原先把敏感性实验的覆盖配置缓存目录（`p<k>_<哈希>`，约 1500 条）也计入，与"固定配置下"的表述不符；改为只统计 `p1/p2/p3`，33,372 → 31,833。
2. 摘要的消融数字原引用旧消融（−18.3%/−6.6%/−7.7%），改为以 c2 为基准的重建消融（−19.8%/−8.3%/−9.2%，并补通信感知与两者同时移除）。
3. 算法 1 步骤 9 与复杂度表的 S4 仍写"12 组候选/大图降为 4"，与 S4 正文的 14/18 个候选、采样与退火不符，已更正。
4. §10.2 与 S4 对大图 P1 兜底的描述互相矛盾（"一次官方核实" vs "两次"），统一为"代理首位核实一次 + 退火首位核实一次，两者取优"。
5. §7.3 对粒度结论的概括改为与新粒度结果一致（问题 2/3 在 [0.12, 0.5] 平台内最优，问题 1 偏好更粗）。
6. 表号重复且乱序（表 10-2 两次、10-8、11-1b）：按出现顺序整理为 10-1…10-5、11-1…11-3。
7. §12.2 第 3 条"多起点是参数网格而非自适应搜索"改为如实描述采样/退火的有限补充；新增第 6 条局限：问题 1 大图兜底与退火耗时（最长 2015 s，42 个配置超 60 s）。
未处理（已知）：参考文献仅 12 条、无相关工作一节（缺增强版文档）；§11.3 敏感性沿用 18 个旧样本。
