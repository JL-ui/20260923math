"""示意图与流程图（不依赖实验数据）。
    ...
    python solution/paper_v2/figs_diagrams.py
"""
...
from figstyle import (C1, C2, C3, DARK, GREY, RED, LIGHT, PIPE_COL, arrow, box, canvas, save, use_sans)
...
def roadmap():
    ...
    box(ax, 1.65, y, 4.7, h, "边界重建 · 核内执行序\n全局申请序 · Belady · FIFO",
        fc="white", ec="#3B6FB6", title="官方评估器逐行解析", title_fc="#3B6FB6", fs=7.8)
    ...
    box(ax, 11.05, y, 4.55, h, "队首跨核 COPY_IN 未就绪\n→ 整核四条流水同时停摆",
        fc="#FFF3F0", ec=RED, title="决定性发现", title_fc=RED, fs=7.8, lw=1.3)
...
def framework():
    ...
    box(ax, 2.75, 3.2, 12.7, 1.1, "保底池：官方评估过的全部方案 ∪ $N-1$ 冠军嵌入 ∪ 问题二、三冠军互投 ∪ 整图单核兜底\n"
        "取 (Makespan, 额外搬运量) 字典序最小者为最终方案", fc="#FFF8EC", ec="#C0632B", fs=7.8, wrap=False)
    arrow(ax, (14.7, 4.75), (14.7, 4.3))
    box(ax, 0.2, 3.2, 2.1, 1.1, "输出\n官方格式方案", fc=BG, ec=EDGE, fs=7.6, wrap=False)
    arrow(ax, (2.75, 3.75), (2.3, 3.75))
    # 三个问题的差异
    difs = [(C1, B1, "问题一（场景 A）", "Task = 子图；同步权重 $\\omega$ = 1000\n偏粗粒度；c0–c13 + 模拟退火"),
            (C2, B2, "问题二（场景 B）", "Task = 核心；$\\omega$ = 500\n+ 单算子子图表调度与采样"),
            (C3, B3, "问题三（场景 B + L2）", "同问题二；代价中命中字节按\n250 B/cycle 折算；与问题二互投")]
    ax.text(8.0, 2.78, "三个问题只在代价权重、候选集合与 L2 折算三处不同", ha="center", fontsize=8,
...
def flows():
    flow("fig_flow_p1", [
        ...
        ("S4 候选择优", "c0–c13 共 14 组候选按图规模裁剪后交官方评估；大图用代价 $F$ 兜底"),
        ("代理驱动退火", "以 Task 级时序模型为适应度，合并 / 拆分 / 迁移 Task，每步检查 Task 图无环"),
        ("★保底池汇总", "官方评估过的全部方案 ∪ $N-1$ 冠军嵌入 ∪ 单核兜底，取 Makespan 最小者"),
        ("输出与检验", "官方格式方案；1～5 核平均加速比曲线、逐用例 Makespan 与额外搬运量"),
    ], {2: (0, "Task 级时序模型：式 (5-1)～(5-4)\n边界搬运解析式与官方输出逐字节一致"),
        3: (0, "引理 4-1：按 $(r,c)$ 编号\n→ 全部硬约束自动成立，搜索中不判环"),
        4: (0, "粒度权衡：式 (5-5)\n场景 A 偏粗粒度（子图数中位数小）"),
        5: (0, "初温取代理值 2%，1500 步几何降温\n取代理前 3 名交官方评估")}, C1, B1, height=15.2)
    flow("fig_flow_p2", [
        ...
        ("S4 候选择优", "c0–c17 共 18 组候选全部交官方评估，覆盖“每核一个子图”到“一算子一子图”"),
        ("优先级采样", "对最优单算子子图候选加 Gumbel 扰动采 16 个，事件模拟代理排序，前 3 名交官方评估"),
        ("★保底池与互投", "官方评估过的方案 ∪ $N-1$ 嵌入 ∪ 问题三冠军互投，取 Makespan 最小者"),
        ("输出与检验", "1～5 核平均加速比曲线、逐用例 Makespan 与额外搬运量"),
    ], {0: (0, "跨核搬运 = 以张量字节为权的\n$(\\lambda-1)$ 超图割：式 (6-2)～(6-4)"),
        2: (0, "评分 = 向上秩 − 驻留增量 + 流水交错\n+ Gumbel 噪声：式 (6-5)，算法 6-1"),
        4: (0, "采样只改核内顺序；层次重排后\n引理 4-1 仍成立")}, C2, B2, height=13.6)
    flow("fig_flow_p3", [
        ...
        ("CAP-LS 求解", "S1–S4 同问题二（含单算子子图与采样），由官方问题三评估器择优"),
        ("★跨问题互投", "问题二冠军放入问题三候选池（反之亦然），保证只读 Cache 配置不劣于无 L2"),
        ("两种配置对比", "无 L2（问题二最终方案）与只读 Cache（问题三最终方案）的加速比与 Cache 加速比"),
        ("机理与灵敏度", "命中 / 未命中来源拆分；L2 容量、带宽灵敏度"),
    ], {1: (0, "命中上界：式 (7-1)～(7-2)\n时序条件：式 (7-3)"),
        2: (0, "代价修正：式 (7-4)\n重复读取从“必须避免”变为“可以承受”"),
        5: (0, "赛题要求：1～5 核对比曲线 +\n相同核数下的 Cache 加速比")}, C3, B3, height=13.6)
