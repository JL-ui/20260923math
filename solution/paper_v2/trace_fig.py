...
"""官方 trace 甘特图：同一用例的“不分层”方案与最终方案，对比 0 号核四条流水的忙闲。

    python solution/paper_v2/trace_fig.py [case]

用官方命令行评估器的 --trace-output 生成 Perfetto trace（写到临时目录，data/ 只读），
统计“四条流水同时空闲而该核仍有未完成算子”的区间（整核停摆），输出
figures/v2/fig_trace.png 与 results/trace_stall_v2.json。
"""
...
def main():
    ...
    final = {}
    import csv
    with open(ROOT / "results" / "final.csv", encoding="utf-8") as f:
    ...
    plans = [("不分层", variants.build(g, "a_no_level", 2, 4)),
             ...
             ("最终方案", json.loads((ROOT / champ["plan_path"]).read_text(encoding="utf-8")))]
    tmp = Path(tempfile.gettempdir()) / "npu_trace_v2"
    ...
    save(fig, "fig_trace")
    (ROOT / "results" / "trace_stall_v2.json").write_text(json.dumps(report, ensure_ascii=False, indent=1),
                                                          encoding="utf-8")
