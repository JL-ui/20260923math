...
"""数据图：全部读 results/ 下的官方评估结果。

    python solution/paper_v2/figs_data.py            # 全部
    python solution/paper_v2/figs_data.py speedup    # 只画名字里含 speedup 的
"""
...
from figstyle import (C1, C2, C3, DARK, GREY, PCOL, PNAME, RED, ROOT, panel_label, save, use_serif)
...
def fig_speedup(p):
    ...
    for x, m in zip(xs, means):
        ax.annotate(f"{m:.2f}", (x, m), textcoords="offset points", xytext=(6, -11), fontsize=8.2,
    ...
    ax.set_xticks(xs)
    ax.set_xlabel("核心数 N")
    ax.set_ylabel("加速比（单核基准 / N 核 Makespan）")
...
def fig_dist():
    ...
    for ax, p in zip(axes, (1, 2, 3)):
        ...
        ax.set_title(PNAME[p], fontsize=9, color=PCOL[p])
        ax.set_xticks(NS)
        ax.set_xlabel("核心数 N")
...
def fig_runtime():
    ...
    for t, lab in ((300, "5 min"), (600, "10 min")):
        ax.axvline(t, color=GREY, lw=0.8, ls=(0, (3, 2)))
        ax.text(t * 1.06, 0.06, lab, fontsize=7.4, color="#555555")
    ax.set_xscale("log")
...
def fig_superlinear():
    ...
    ax.axhline(4, color=GREY, lw=0.9, ls=(0, (4, 3)))
    ax.text(x.max() * 0.9, 4.08, "加速比 = 核心数 4", fontsize=7.6, color="#555555", ha="right")
    ax.axvline(2e4, color="#BBBBBB", lw=0.7)
    ax.text(1.25e4, s.max() * 0.98, "单核基准\n无换入换出", fontsize=7.4, color="#555555", ha="center", va="top")
    ax.set_xscale("log")
    ax.set_xlabel("单核基准自身的换入换出字节数（左侧竖线左边为无换入换出的用例）")
    ax.set_ylabel("问题二 N=4 加速比")
    for c, lab in ((C1, "ρ_max < 0.2"), (C3, "0.2 ≤ ρ_max < 0.5"), (C2, "ρ_max ≥ 0.5")):
        ax.scatter([], [], c=c, s=18, label=lab)
    ax.legend(loc="lower right", fontsize=7.6, title="最大连通分量占比", title_fontsize=7.6)
...
def fig_traffic():
    ...
    for i, p in enumerate((1, 2, 3)):
        ax.text(i * 4.6 + 1.5, -0.33, PNAME[p], ha="center", va="top", fontsize=8.2, color=PCOL[p],
...
def fig_p3_compare():
    ...
    ax.annotate(f"{ro[-1]:.2f}", (5, ro[-1]), textcoords="offset points", xytext=(-4, 7), fontsize=7.8,
                color=C3, ha="right")
    ax.annotate(f"{no_l2[-1]:.2f}", (5, no_l2[-1]), textcoords="offset points", xytext=(-4, -12), fontsize=7.8,
                color=C2, ha="right")
    ax.set_xticks(xs)
    ax.set_xlabel("核心数 N")
    ...
    panel_label(ax, "(a)")
    ax = axes[1]
    ax.plot(xs, gain_final, color=C3, marker="o", ms=4.6, lw=1.6, label="各自最终方案")
    ax.plot(xs, gain_same, color=C3, marker="^", ms=4.2, lw=1.1, ls=(0, (3, 2)), label="同一方案")
    ax.axhline(1, color=GREY, lw=0.8)
    for x, g in zip(xs, gain_final):
        ax.annotate(f"{g:.3f}", (x, g), textcoords="offset points", xytext=(0, -12), fontsize=7.2, ha="center",
                    color=C3)
    ax.set_xticks(xs)
    ax.set_xlabel("核心数 N")
    ax.set_ylabel("Cache 加速比")
    ax.legend(loc="upper left", fontsize=7.6)
    panel_label(ax, "(b)")
    ax = axes[2]
    ax.bar(xs, hit, color=C3, alpha=0.75, width=0.6, edgecolor="white")
...
def fig_l2_case():
    ...
    axes[0].set_ylabel("逐用例 Cache 加速比")
    fig.tight_layout(w_pad=0.6)
...
def fig_l2_sources():
    ...
    panel_label(ax, "(a)")
    ax = axes[1]
    parts = [("miss_first_miss", "首次读入（不可避免）", "#9AA5B1"),
             ("miss_concurrent_first_read", "并发首读", C2),
             ("miss_fifo_evicted", "被 FIFO 淘汰后再读", RED)]
    bottom = np.zeros(len(ns))
...
def fig_baselines():
    ...
    names = [("random", "B1 随机", "#9AA5B1", "v"), ("topo", "B2 拓扑等分", "#B8860B", "D"),
             ("balance", "B3 负载均衡", "#8E6FB0", "^"), ("comm", "B4 通信聚类", "#5A8FBF", "s")]
    ...
    for ax, p in zip(axes, (1, 2, 3)):
        ...
        ax.plot([1] + NS, ys, color=PCOL[p], marker="o", ms=4.6, lw=2.0, label="CAP-LS（本文）")
    ...
    fig.legend(h[:4] + [plt.Line2D([], [], color=DARK, marker="o", lw=2)], lab[:4] + ["CAP-LS（本文）"],
...
def fig_ablation():
    ...
    fig, ax = plt.subplots(figsize=(14.5 * CM, 7.4 * CM))
    ...
    for j, p in enumerate((1, 2, 3)):
        ...
        for v, s_, yv in zip(vals, sig, yy):
            ...
            ax.text(v + (0.8 if v >= 0 else -0.8), yv, f"{v:+.1f}%" + ("*" if s_ else ""), va="center",
                    ha="left" if v >= 0 else "right", fontsize=6.6, color=DARK)
...
def fig_granularity():
    ...
    ax.text(0.37, ax.get_ylim()[1] - 0.01, "默认 β = 0.35", fontsize=7.4, color="#555555", va="top")
    ax.set_xscale("log")
...
def fig_sensitivity():
    ...
    ax = axes[7]
    ax.axis("off")
    ax.text(0.02, 0.62, "纵轴 > 1 表示比题设配置更快\n红圈为题设配置\n18 个用例几何平均，N=4", fontsize=7.4,
...
def fig_bounds():
    ...
    ax.set_xlabel("最终方案 Makespan / 理论下界（N=4）")
    ax.set_ylabel("累计比例")
    ax.legend(loc="lower right", fontsize=7.6)
    save(fig, "fig_bounds")
...
def fig_proxy():
    ...
    ax.bar(x - w / 2, lv, w, color="#9AA5B1", label="解析代理 F")
    ...
    x = np.arange(3)
    ax.bar(x - w / 2, lv, w, color="#9AA5B1", label="解析代理 F")
    ax.bar(x + w / 2, sv, w, color=C2, label="事件模拟代理")
    for i in range(3):
    ...
    ax.set_ylabel("官方最优候选落入代理前 K 名的比例")
    ax.set_ylim(0, 1.12)
...
def fig_struct():
    f = feats.loc[CASES]
    s = sp(2, 4)
    items = [(f.largest_component_frac, "最大连通分量占比 ρ_max", False),
...
def fig_heatmap():
    ...
    for ax, p in zip(axes, (1, 2, 3)):
        ...
        im = ax.imshow(M, aspect="auto", cmap="RdYlBu", vmin=0.2, vmax=1.4, interpolation="nearest")
    ...
    axes[-1].set_xticklabels(["ρ_max < 0.2", "0.2 ≤ ρ_max < 0.5", "ρ_max ≥ 0.5"], fontsize=7.8)
