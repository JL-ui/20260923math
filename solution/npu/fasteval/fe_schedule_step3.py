# 本文件复制自赛题材料包提供的评估程序 code/schedule_step3.py，
...
from .fe_contest_io import emit_error
...
def step3_simulation(ext_graph, capacity, bandwidth, max_iter=100000):
    ...
    # (c) "全部完成"检查：按固定顺序 status_order 用单调指针代替每轮全量扫描。
    # 每个 op 的状态只会单调地推进到 'done' 并保持，因此指针在任意固定顺序下
    # 都能正确判定"是否全部完成"，判定结果与原 all(...) 扫描完全相同。
    status_order = list(op_status)
    done_cursor = 0
    while True:
        iteration += 1
        ...
        retire_step()
        while (done_cursor < len(status_order)
               and op_status[status_order[done_cursor]] == 'done'):
            done_cursor += 1
        if done_cursor == len(status_order):
            break
        issue_step()
