# 本文件复制自赛题材料包提供的评估程序 code/multicore_cut_evaluate_problem_1.py，
...
from .fe_evaluation_validation import validate_parameters, validate_task_order

from .fe_schedule_step1 import step1_schedule
from .fe_schedule_step2 import _build_extended_graph, step2_spill_insertion
from .fe_schedule_step3 import (
...
def read_scene_a_config(config_path):
    ...
    from .fe_evaluation_validation import read_required_settings
...
def evaluate_scene_a(graph_json, plan, bandwidth, capacity,
    ...
    done_ptr = {task_id: 0 for task_id in tasks}
    ...
    def retire(now):
        ...
        for task_id, status in list(task_status.items()):
            if status != 'active':
                continue
            # (c) 按 seq 顺序找第一个未完成操作：用单调指针代替每次全量扫描，
            # 保证判定结果与原线性扫描完全相同（seq 覆盖完整且只增不减）。
            seq = tasks[task_id]['seq']
            ptr = done_ptr[task_id]
            while ptr < len(seq) and op_status[key(task_id, seq[ptr])] == 'done':
                ptr += 1
            done_ptr[task_id] = ptr
            if ptr == len(seq):
...
if __name__ == '__main__':
    from .fe_contest_io import run_problem_cli
