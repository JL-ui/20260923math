"""v3 示意图：由 solution/paper_v2/figs_diagrams.py 复制，只重画技术路线图、CAP-LS 框架与三个流程图。

    python paper/v3/tools/figs_diagrams_v3.py

与 v2 的差别：去掉“保底池 / 互投”等离线汇总步骤（正式求解流程不含这些步骤），
流程图批注不再写公式编号，措辞按 v3 术语修改；输出到 figures/v3/。
"""
...
figstyle.OUT = figstyle.ROOT / "figures" / "v3"
...
if __name__ == "__main__":
    ...
    if "Microsoft YaHei" not in {f.name for f in _fm.fontManager.ttflist}:
        _orig = figstyle.use_sans

        def _sans_fallback():
            _orig()
            plt.rcParams["font.family"] = ["Liberation Sans", "Noto Sans CJK SC"]
        ...
        figstyle.use_sans = _sans_fallback
        # 无 Windows 字体时 mathtext 无法混排中文：把 $...$ 换成 Unicode 近似写法（仅影响云端预览图）
        import re as _re
        _G = {"theta": "θ", "sigma": "σ", "beta": "β", "lambda": "λ", "omega": "ω", "delta": "δ",
              "pi": "π", "rho": "ρ", "le": "≤", "ge": "≥", "to": "→", "in": "∈", "cdot": "·"}
        ...
        def _plain(t):
            def one(m):
                ...
                x = _re.sub(r"\\[,;! ]", " ", x)
                sub = dict(zip("0123456789aehijklmnoprstuvxN", "₀₁₂₃₄₅₆₇₈₉ₐₑₕᵢⱼₖₗₘₙₒₚᵣₛₜᵤᵥₓₙ"))
                x = _re.sub(r"_\{([^}]*)\}|_(\w)", lambda g: "".join(sub.get(ch, ch) for ch in (g.group(1) or g.group(2))), x)
                x = x.replace("{", "").replace("}", "").replace("^", "")
                return x
            return _re.sub(r"\$([^$]*)\$", one, t) if isinstance(t, str) else t

        _box = box

        def _box_plain(ax, x, y, w, h, text, *a, **k):
            if "title" in k:
                k["title"] = _plain(k["title"])
            return _box(ax, x, y, w, h, _plain(text), *a, **k)
        globals()["box"] = _box_plain
    figstyle.use_sans()
    roadmap()
    framework()
    flows()
