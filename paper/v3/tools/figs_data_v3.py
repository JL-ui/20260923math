...
if "SimSun" not in _have:
    plt.rcParams["font.family"] = ["Times New Roman", "Liberation Serif", "Noto Serif CJK SC"]
    plt.rcParams["mathtext.rm"] = "Noto Serif CJK SC"
    plt.rcParams["mathtext.it"] = "Liberation Serif:italic"
    plt.rcParams["mathtext.bf"] = "Liberation Serif:bold"
    plt.rcParams["mathtext.cal"] = "Liberation Serif:italic"
    plt.rcParams["mathtext.sf"] = "Liberation Sans"
    plt.rcParams["mathtext.tt"] = "Liberation Mono"
CM = 1 / 2.54
NS = [2, 3, 4, 5]
...
def fig_p3_compare():
    ...
    panel_label(ax, "(a)")
    ax = axes[1]
    ax.plot(xs, gain_final, color=C3, marker="o", ms=4.6, lw=1.6, label="配置最优比较")
    ...
    ax.set_xticks(xs)
    ax.set_xlabel("核心数 N")
    ax.set_ylabel("无 L2 / 只读 Cache 的 Makespan 比")
    ax.legend(loc="upper left", fontsize=7.6)
    panel_label(ax, "(b)")
    ax = axes[2]
