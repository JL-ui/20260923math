...
**实验方面**，在官方给定的全部 100 个测试用例与固定配置下，用官方评估脚本完成 @@TOTAL_EVALS@@ 次评估。CAP-LS 在 $N=2,3,4,5$ 下的平均加速比（按赛题定义取算术平均）为：问题 1 @@P1_CURVE@@；问题 2 @@P2_CURVE@@；问题 3 @@P3_CURVE@@。相同切图方案下，只读 L2 相对无 L2 基线的 Makespan 加速比在 4 核时达到 @@L2_GAIN_N4@@，平均按字节命中率 @@HIT_N4@@。与随机、拓扑等分、负载均衡、通信聚类四类基线相比，CAP-LS 在 4 核问题 2 上的平均加速比分别提升 @@GAIN_VS_BASE@@。以候选 c2 为基准的消融实验表明同步层次分层是贡献最大的单一机制（移除后问题 1 的平均加速比变化 @@ABL2_REL_no_level_P1@@、问题 2 变化 @@ABL2_REL_no_level_P2@@、问题 3 变化 @@ABL2_REL_no_level_P3@@），通信感知切图次之；同时移除两者，问题 2 的变化达 @@ABL2_REL_no_level_no_comm_P2@@。算法对 $|V_{\mathrm{op}}|\le 3.6\times10^4$ 的最大用例单候选求解时间中位数 @@RUNTIME_MED@@ s。
...
> 9. 对全部候选参数重复步骤 2–8（12 组基础候选，加 ALAP 层指派与单算子子图候选，共 14 或 18 组），
>    再加入优先级采样（问题 2/3）或模拟退火（问题 1）产生的方案，用官方评估器取 Makespan 最小者。
...
场景 B 中把更多子图放在同一核可减少跨核搬运，但跨越多个子图的驻留数据会持续占用 L1/UB，压缩中间子图的可用空间。本文用**块计算量上限 $\beta W/N$** 同时控制这两端：$\beta$ 小 ⇒ 子图多 ⇒ 驻留窗口短、换入换出少，但跨核边多；$\beta$ 大 ⇒ 反之。§11.2 的粒度敏感性实验给出了 $\beta$ 的经验取值范围：问题 2、3 在 $\beta\in[0.12,0.5]$ 的平台内最优，问题 1 则偏好更粗的粒度。
...
| S4 多起点 | $\times 14$（问题 1）或 $\times 18$（问题 2/3）候选，另加至多 3 个采样/退火方案 | 候选间方案哈希去重；问题 1 大图的候选裁剪见 §6.3 |
...
3. **多起点以参数网格为主，自适应搜索只是有限的补充**。除网格外，本文只在冠军附近做少量优先级采样（问题 2/3）与模拟退火（问题 1）；更强的做法是用图特征预测最优 $\beta$，本文只给出了相关性分析（图 12）而未做回归建模。
...
6. **问题 1 的大图与退火耗时未被完全控制**。case_014/072/076/091 四个大图的问题 1 只用"代理排序 + 两次官方核实"，没有评估完整候选网格；退火耗时取决于搜索轨迹与 Task 规模，最长达 @@SA_MAX_S@@ s，有 @@SA_OVER@@ 个配置超过 60 s 的预算（§10.2）。这些用例上的问题 1 成绩可能低于完整评估候选网格的结果。
